import{n as e}from"./layout-BZDOoLye.js";e({title:`Bunker - AI Models`,description:`Browse and deploy AI models built by Bunker for drug discovery, protein folding, genomics, and beyond.`,activePath:`/ai-models`,maxWidth:740,content:`
  <header class="hero">
...
state: active_regulatory</div>
  ...
  </div>`});var t=document.querySelectorAll(`.filters button`),n=document.querySelectorAll(`.model-card`),r=document.getElementById(`model-count`),i=document.getElementById(`no-results`);function a(e){let t=0;n.forEach(n=>{let r=e===`all`||n.dataset.category===e;n.style.display=r?``:`none`,r&&t++}),r&&(r.textContent=`Showing ${t} model${t===1?``:`s`}`),i&&(i.style.display=t===0?`block`:`none`)}t.forEach(e=>{e.addEventListener(`click`,()=>{t.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),a(e.dataset.filter)})}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.textContent;e.textContent=`Coming soon`,e.disabled=!0,setTimeout(()=>{e.textContent=t,e.disabled=!1},2e3)})});